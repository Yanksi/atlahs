...
static nvtxEventAttributes_t eventAttrib = {0};
#endif
    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      if (sub->transmitted < sub->posted && sub->transmitted < sub->done + NCCL_STEPS) {
        ...
        if (connFifo[buffSlot].size != -1 && (*recvTail > tail || p == NCCL_PROTO_LL)) {
          ...
          if (ready) {
            ...
            #if defined(ENABLE_NET_NVTX)
              // Remember the size for SendTest marks
            ...
            #if defined(ENABLE_NET_NVTX)
            {
              struct sendNetResources* r = (struct sendNetResources*)sub->connection->transportResources;
              char nvtxMsg[256];
              snprintf(nvtxMsg, sizeof(nvtxMsg),
                      "ncclNetIsend(): Data_Size: %d, Sender_Rank: %d, Receiver_Rank: %d, Channel_ID: %d, Sequence: %d",
                      size, r->tpRank, r->tpRemoteRank, sub->channelId, (int)(sub->base + sub->posted));
              ...
              // First SendTest not seen yet for this slot
      ...
      if (sub->done < sub->transmitted) {
        ...
        #if defined(ENABLE_NET_NVTX)
        {
          ...
          eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
          eventAttrib.colorType = NVTX_COLOR_ARGB;
          eventAttrib.message.ascii = (char*)"ncclNetSendTest()";
          eventAttrib.color = colors[0];
          nvtxRangePushEx(&eventAttrib);

          if (sub->nvtxSendFifo[buffSlot] == 0) {
            ...
            snprintf(nvtxMsg, sizeof(nvtxMsg),
                    ...
                    sub->nvtxSizesFifo[buffSlot], r->tpRank, r->tpRemoteRank, sub->channelId, (int)(sub->base + sub->posted));

            nvtxEventAttributes_t ea = eventAttrib;
            ea.message.ascii = nvtxMsg;
            nvtxMarkEx(&ea);
          }
        }
        #endif
  ...
  if (args->state == ncclProxyOpProgress) {
      ...
      if (subCount) {
        ...
        if (*requestPtr) {
          ...
          for (int i=0; i<subGroup->groupSize; i++) {
            ...
            #if defined(ENABLE_NET_NVTX)
            {
              // One flat mark per posted irecv (like 2.20)
              char nvtxMsg[256];
              snprintf(nvtxMsg, sizeof(nvtxMsg),
                      "ncclNetIrecv(): Sender_Rank: %d, Receiver_Rank: %d, Channel_ID: %d, Sequence: %d",
                      resources->tpRemoteRank, resources->tpRank, sub->channelId, (int)step);
              ...
              eventAttrib.colorType = NVTX_COLOR_ARGB;
              eventAttrib.message.ascii = nvtxMsg;
              eventAttrib.color = colors[2]; // same palette as 2.20 blocks

              nvtxMarkEx(&eventAttrib);

              // Mark first-time recv-test for this step as “not yet seen”
    ...
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      ...
      if (subGroup->posted > subGroup->received) {
        ...
        #if defined(ENABLE_NET_NVTX)
        {
          // Range around RecvTest
          eventAttrib.version = NVTX_VERSION;
          eventAttrib.size = NVTX_EVENT_ATTRIB_STRUCT_SIZE;
          eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
          eventAttrib.colorType = NVTX_COLOR_ARGB;
          eventAttrib.message.ascii = (char*)"ncclNetRecvTest()";
          eventAttrib.color = colors[3];
          nvtxRangePushEx(&eventAttrib);

          // Emit one "first-test" mark per sub (only once per step)
          for (int i = 0; i < subGroup->groupSize; i++) {
            struct ncclProxySubArgs* sub = subGroup + i;
            if (sub->nvtxRecvFifo[step%NCCL_STEPS] == 0) {
              sub->nvtxRecvFifo[step%NCCL_STEPS] = 1;
              struct recvNetResources* r = (struct recvNetResources*)subGroup->connection->transportResources;

              char nvtxMsg[256];
              snprintf(nvtxMsg, sizeof(nvtxMsg),
                      "ncclNetRecvTest(): Data_Size: %d, Sender_Rank: %d, Receiver_Rank: %d, Channel_ID: %d, Sequence: %d",
                      sizes[i], r->tpRemoteRank, r->tpRank, sub->channelId, (int)step);
        ...
        #if defined(ENABLE_NET_NVTX)
        if (!done) {
          // Keep one open range (single push → single pop)
          nvtxRangePop();
        }
        #endif
        ...
        if (done) {
          ...
          for (int i=0; i<subGroup->groupSize; i++) {
            ...
            #if defined(ENABLE_NET_NVTX)
            {
              ...
              // Close the RecvTest range when done
              nvtxRangePop();
            }
            #endif
