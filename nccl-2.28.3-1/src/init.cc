...
static nvtxEventAttributes_t eventAttrib_channel;
static nvtxEventAttributes_t eventAttrib_init;
#endif
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  for (int c=0; c<comm->nChannels; c++) {
    ...
    INFO(NCCL_GRAPH, "Ring %02d : %d -> %d -> %d", c, comm->channels[c].ring.prev, comm->rank, comm->channels[c].ring.next);
      ...
      // Also mirror the per-tree event that 2.20 emitted alongside Rings
      struct ncclTree* tree_ = &comm->channels[c].tree;
      snprintf(nvtxMsg_Tree, sizeof(nvtxMsg_Tree),
              "commHash 0x%llx Trees [%d] %d/%d/%d->%d->%d pid %d",
              (unsigned long long)comm->commHash,
              c,
              tree_->down[0], tree_->down[1], tree_->down[2],
              comm->rank, tree_->up,
              (int)pid);
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  #if defined(ENABLE_INIT_NVTX)
  {
    // Match 2.20: one "Init" range per init call.
    // Use both commHash and (if available) commIdHash in the message,
    // just like before. In split-parent flow, commIdHash is 0 in 2.28.
    extern unsigned long long commIdHash; // already in this function’s scope
    char nvtxMsg_Init[256];
    snprintf(nvtxMsg_Init, sizeof(nvtxMsg_Init),
            "Init START commHash 0x%llx rank %d nranks %d ncclUniqueIdHash 0x%llx pid %d",
            (unsigned long long)comm->commHash,
