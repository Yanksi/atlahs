  ...
  while (nPlanColls!=0 && !ncclIntruQueueEmpty(&planner->collTaskQueue)) {
    ...
    if (task->isCollnet) {
      ...
      #if defined(ENABLE_ENQUEUE_NVTX)
      {
        ...
        pid_t pid = getpid();
        int nWarps = task->nWarps;  // was: task->nThreads / WARP_SIZE
        unsigned long count = (unsigned long)task->count;
        unsigned long chunkCount = (unsigned long)(chunkSize / ncclTypeSize(task->datatype));
        unsigned long workCount = count;           // CollNet: whole op per work item
        unsigned long lastChunkCount = (chunkCount && (workCount % chunkCount)) ? (workCount % chunkCount) : chunkCount;
        unsigned long workOffset = 0UL;            // no per-part offset here
        uintptr_t sendbuff = (uintptr_t)task->sendbuff;
        uintptr_t recvbuff = (uintptr_t)task->recvbuff;
        ...
        snprintf(nvtxMsg_WorkElemColl, sizeof(nvtxMsg_WorkElemColl),
                ...
                nWarps, count, chunkCount, workCount, lastChunkCount, workOffset,
                (unsigned long)sendbuff, (unsigned long)recvbuff, (int)pid);
...
static ncclResult_t addP2pToPlan(
  ...
  #if defined(ENABLE_ENQUEUE_NVTX)
{
  // Match 2.20: two flat NVTX marks, one for RECV (p2pType=1) and one for SEND (p2pType=2).
  pid_t pid = getpid();
  int nWarps = NCCL_MAX_NTHREADS / WARP_SIZE;

  // RECV
  {
    size_t ubytes = (recvBytes == -1) ? 0 : (size_t)recvBytes;
    char nvtxMsg_WorkElemP2P[256];
    uint32_t countLo32 = (uint32_t)ubytes;
    uint32_t countHi32 = (uint32_t)(((unsigned long long)ubytes) >> 32);
  ...
  // SEND
  {
    size_t ubytes = (sendBytes == -1) ? 0 : (size_t)sendBytes;
    char nvtxMsg_WorkElemP2P[256];
    uint32_t countLo32 = (uint32_t)ubytes;
    uint32_t countHi32 = (uint32_t)(((unsigned long long)ubytes) >> 32);
    snprintf(nvtxMsg_WorkElemP2P, sizeof(nvtxMsg_WorkElemP2P),
...
#if defined(ENABLE_ENQUEUE_NVTX)
{
  ...
  // Match 2.20 fields (use %p for stream; it's not available here so pass NULL).
  void*  streamPtr = NULL;
  size_t data_size = (size_t)info->count * ncclTypeSize(info->datatype);
  int    type_size = (int)ncclTypeSize(info->datatype);

  // Local chunking params are in scope in this function:
  //   int stepSize, chunkSteps, sliceSteps; and we finalized proxyOp->chunkSize.
  int    chunkSizeVal = (int)proxyOp->chunkSize;
  int    chunkCount   = chunkSizeVal / type_size;
    ...
    if (comm->nRanks == 1) {
      ...
      #if defined(ENABLE_ENQUEUE_NVTX)
      {
        ...
        snprintf(nvtxMsg_Check, sizeof(nvtxMsg_Check),
                "Not executed in ncclkernel: comm->nRanks == 1 pid %d", pid);
